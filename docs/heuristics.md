...
> 💡 **Decision:** Neutral config files are *filtered before voting*, not classified as their own type. A `.gitignore` tweak shipped alongside docs is still a docs commit; a `.gitignore` tweak shipped alongside src is still a feat. Only when the *entire* diff is neutral does the type become `chore`. Mirror of `scope.ts` rung 4: drop noise, classify on what's left.
